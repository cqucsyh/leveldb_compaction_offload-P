#!/usr/bin/env bash
# Run stress simulation for tb_sstable_engine_stress
# Tests 16-block-pair continuous processing with clear/restart and split modes
set -euo pipefail
cd "$(dirname "$0")"
...
python3 gen_test_sstable_stress.py

echo "=== Compiling ==="
iverilog -g2012 -Wall -Wno-timescale \
    -I "${RTL}" \
    -o tb_sstable_engine_stress.vvp \
    tb_sstable_engine_stress.v \
...
vvp tb_sstable_engine_stress.vvp
