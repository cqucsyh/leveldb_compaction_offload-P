#!/usr/bin/env bash
# Run simulation for tb_sstable_asym (asymmetric block-pair test)
set -euo pipefail
cd "$(dirname "$0")"
...
if [ ! -f fixtures/src0_asym_real.memh ]; then
    echo "Generating asymmetric SSTable fixtures..."
    python3 gen_test_sstable_asymmetric.py
fi

iverilog -g2012 -Wall -Wno-timescale \
    -I "${RTL}" \
    -o tb_sstable_asym.vvp \
    tb_sstable_asym.v \
