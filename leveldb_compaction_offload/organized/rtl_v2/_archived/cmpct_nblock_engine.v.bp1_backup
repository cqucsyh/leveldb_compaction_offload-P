...
module stage4_real_internal_key_two_way_merge_stage5_nblock_top #(
    ...
    localparam [3:0] ST_PIPE_CLEAR  = 4'd10;  // front-clear for pipelined restart
    localparam [3:0] ST_PIPE_START  = 4'd11;  // front-start for pipelined restart
    ...
    reg       pipeline_restart_r;  // for ST_POP → ST_PIPE_CLEAR routing
    ...
    reg [63:0]                               block_offset_mem           [0:MAX_BLOCK_PAIRS-1];
    reg [15:0]                               last_key_len_mem           [0:MAX_BLOCK_PAIRS-1];
    reg [(STAGE5_MAX_KEY_BYTES*8)-1:0]       last_key_bytes_mem         [0:MAX_BLOCK_PAIRS-1];
    ...
    // OPT-P1c: desc_ready is high in ST_POP state (stream mode)
    assign desc_ready = (state == ST_POP);
    ...
    // Flatten stored per-block metadata for assembler
    wire [MAX_BLOCK_PAIRS*64-1:0]                      block_offset_vec;
    wire [MAX_BLOCK_PAIRS*32-1:0]                      block_size_vec;
    wire [MAX_BLOCK_PAIRS*16-1:0]                      last_key_len_vec;
    wire [MAX_BLOCK_PAIRS*(STAGE5_MAX_KEY_BYTES*8)-1:0] last_key_bytes_vec;
    genvar bgi;
    generate
        for (bgi = 0; bgi < MAX_BLOCK_PAIRS; bgi = bgi + 1) begin : g_block_meta_vec
            assign block_offset_vec [(bgi*64) +: 64]                      = block_offset_mem[bgi];
            assign block_size_vec   [(bgi*32) +: 32]                      = dst_output_block_bytes_mem[bgi];
            assign last_key_len_vec [(bgi*16) +: 16]                      = last_key_len_mem[bgi];
            assign last_key_bytes_vec[(bgi*(STAGE5_MAX_KEY_BYTES*8)) +: (STAGE5_MAX_KEY_BYTES*8)] = last_key_bytes_mem[bgi];
        end
    endgenerate

    cmpct_sstable_assembler #(
        .AXI_ADDR_WIDTH(AXI_ADDR_WIDTH),
        .AXI_DATA_WIDTH(AXI_DATA_WIDTH),
        .AXI_ID_WIDTH(AXI_ID_WIDTH),
        .MAX_BURST_LEN(MAX_BURST_LEN),
        ...
        .block_offset_vec(block_offset_vec),
        .block_size_vec(block_size_vec),
        .last_key_len_vec(last_key_len_vec),
        .last_key_bytes_vec(last_key_bytes_vec),
        .busy(asm_busy),
        .done(asm_done),
        .error(asm_error),
    ...
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            for (i = 0; i < MAX_BLOCK_PAIRS; i = i + 1) begin
                ...
                block_offset_mem[i]           <= 64'd0;
                last_key_len_mem[i]           <= 16'd0;
                last_key_bytes_mem[i]         <= {(STAGE5_MAX_KEY_BYTES*8){1'b0}};
            end
            ...
            case (state)
                ...
                ST_POP: begin
                    if (desc_valid) begin
                        ...
                        // OPT-BP1: route to pipeline clear if this is a pipelined restart
                        state                     <= pipeline_restart_r ? ST_PIPE_CLEAR : ST_CLEAR;
                    end
                end
                ...
                ST_WAIT: begin
                    if (inner_error) begin
                        ...
                        block_offset_mem[blocks_in_current_sst_r]           <= running_offset_r;
                        last_key_len_mem[blocks_in_current_sst_r]           <= inner_stage5_last_key_len;
                        last_key_bytes_mem[blocks_in_current_sst_r]         <= inner_stage5_last_key_bytes;
                        ...
                        if (USE_DESC_STREAM ? desc_is_last_r
                            ...
                            // OPT-BP1: pipeline — front_clear/start next pair while write continues
                            pipeline_wr_pending <= 1'b1;
                            pipeline_restart_r  <= 1'b1;
                            active_block_index  <= active_block_index + 32'd1;
                            state               <= USE_DESC_STREAM ? ST_POP : ST_PIPE_CLEAR;
                        end
                    end
                end
                ...
                // OPT-BP1: Pipeline front-clear — reset front-end for next pair
                ST_PIPE_CLEAR: begin
                ...
                // OPT-BP1: Pipeline front-start — start front-end for next pair
                ST_PIPE_START: begin
                ...
                ST_SPLIT_RESET: begin
                    ...
                    // Clear per-block metadata for next sub-SSTable
                    for (i = 0; i < MAX_BLOCK_PAIRS; i = i + 1) begin
                        dst_output_block_bytes_mem[i] <= 32'd0;
                        block_offset_mem[i]           <= 64'd0;
                        last_key_len_mem[i]           <= 16'd0;
